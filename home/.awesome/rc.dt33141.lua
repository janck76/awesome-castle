...
package.path = home .. "/.awesome/?.lua;" .. package.path
require("jc")
-- }}}
--
-- file = io.open('/tmp/luapath', 'w')
-- io.output(file):write(package.path)
...
-- JCK -  test av callback
--   naughty.notify({ preset = naughty.config.presets.critical,
--                     title = "Oops, there were errors during startup!",
--                     run = function() awful.util.spawn 'zenity --info' end,
--                     text = awesome.startup_errors })
...
mymux      = terminal .. " -name URxvt -title tmux -e tmuxinator mymux"
...
-- gpg passord via ~/.xintirc
--mailwidget = wibox.widget.background(lain.widgets.imap({
--    timeout  = 30,
--    server   = "imap.gmail.com",
--    mail     = "janck76@gmail.com",
--    password = "$HOME/bin/janck76pw 0",
--    settings = function()
--        if mailcount > 0 then
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "End",
        function() 
            awful.util.spawn(home .. "/script/lockdesktop.sh")
        end),

    --awful.key({ modkey, }, "z", function () os.execute("urxvt_theme") end),
    ...
    -- Standard program
    awful.key({ modkey,           }, "Return", function () awful.util.spawn(mymux) end),
...
awful.rules.rules = {
    ...
    { rule = { class = "URxvt" },
          -- JCK
          properties = { opacity = 0.99, maximized_vertical = true, maximized_horizontal = true } },
